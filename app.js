...
function parseMeeting(text) { const match=text.trim().match(/^([MTWRFSU]+)\s+(.+)$/i); if(!match)return null; const range=match[2].split(/\s*-\s*/),start=parseTime(range[0]),end=parseTime(range[1]); if(start===null||end===null||range.length!==2)return null; return {days:[...match[1].toUpperCase()].map(d=>dayMap[d]).filter(d=>d!==undefined),start,end}; }
function parseSchedule() {
  const parsed=[],errors=[];
  input.value.split(/\r?\n/).forEach((line,i)=>{
    if(!line.trim())return; const p=line.split('|').map(x=>x.trim()),lecture=parseMeeting(p[1]||''),discussion=parseMeeting(p[2]||'');
    if(p.length<4||!lecture){errors.push(i+1);return;}
    const offset=discussion?1:0, rooms=(p[3+offset]||'Room not listed').split('/').map(room=>room.trim());
    ...
    const finalTime=p[5+offset]||'', finalStart=parseTime(finalTime.split(/\s*-\s*/)[0]||''), finalEnd=parseTime(finalTime.split(/\s*-\s*/)[1]||'');
    parsed.push({id:`${p[0]}-${i}-${Date.now()}`,code:p[0],meetings,teacher:p[2+offset]||'Teacher not listed',finalDate:p[4+offset]||'',finalStart,finalEnd,finalRoom:p[6+offset]||rooms[0]||'Room not listed',enrolled:false});
  });
